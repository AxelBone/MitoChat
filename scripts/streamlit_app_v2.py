...
Tâche:
Réécris la question actuelle pour qu’elle soit autonome en respectant strictement les contraintes.
Rappelle‑toi: aucune invention. Garde la langue d’origine. Conserve à l’identique gènes/variants/sigles.
Sortie (strictement): {open_tag}…{close_tag}
""".strip()
...
rewrite_prompts = load_rewrite_prompts(lang="fr", base_dir="../prompts")
...
def build_messages_with_history(
    ...
    user_prompt = (
        "Utilise uniquement le contexte documentaire pour répondre. "
        "Si l’information n’est pas présente, dis que tu ne sais pas.\n"
        "Cite les sources avec des numéros entre crochets [1], [2], etc.\n\n"
        f"Contexte de conversation (récent):\n{conversation}\n\n"
...
def main():
    ...
    with st.sidebar:
        ...
        # Query rewritting
        st.divider()
        use_rewrite = st.checkbox("Use query rewriting (French)", value=True)
    ...
    for i, m in enumerate(st.session_state.chat):
        with st.chat_message(m["role"]):
            ...
            #                 # if len(txt) > preview_len and st.checkbox("Show full chunk text", key=f"full_{i}_{j}"):
            #                 #     st.text_area("Full chunk", txt[:50_000], height=250)
            #                 txt = h.get("text") or ""
            #                 st.text_area("Chunk", txt[:50_000], height=250)
            #                 st.markdown("---")
    ...
    if user_input:
        ...
        # 2) Show it immediately in the chat (grey user bubble)
        with st.chat_message("user"):
            st.markdown(user_input)

        # 3) Compute rewrite (do NOT display it yet)
        ...
        # 4) Retrieval (spinner shows below the just-rendered user message)
        with st.spinner("Collecte des documents pertinents…"):
        ...
        # 5) Generate answer (stream) and only then attach the rewrite to the user turn
        with st.chat_message("assistant"):
            if not hits:
                msg = "Je ne trouve pas de passages pertinents. Essayez de reformuler."
                st.markdown(msg)
                st.session_state.chat[user_idx]["rewrite"] = standalone_q
                st.session_state.chat[user_idx]["rewrite_debug"] = rw_debug
                st.session_state.chat.append({"role": "assistant", "content": msg, "hits": []})
                st.rerun()

            messages = build_messages_with_history(user_input, hits, system_prompt, num_ctx, st.session_state.chat)
            ...
            if provider == "llama_cpp":
                ...
                # Attach rewrite AFTER we have the answer
                st.session_state.chat[user_idx]["rewrite"] = standalone_q
                st.session_state.chat[user_idx]["rewrite_debug"] = rw_debug
                st.session_state.chat.append({"role": "assistant", "content": full_text, "hits": hits})
                ...
                st.markdown(answer or "Generation failed.")
                st.session_state.chat[user_idx]["rewrite"] = standalone_q
                st.session_state.chat[user_idx]["rewrite_debug"] = rw_debug
                st.session_state.chat.append({"role": "assistant", "content": answer or "", "hits": hits})
